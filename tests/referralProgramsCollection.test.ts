...
describe("Referral Programs Collection v2", () => {
  ...
  it("should preserve top-level maxAmount when provided", async () => {
    const result = await beforeChangeHook({
      data: {
        maxAmount: 40,
        commissionRules: [
    ...
    expect(result.commissionRules[0]?.totalCommission?.maxAmount).toBeUndefined();
  });

  it("should normalize missing top-level minOrderAmount and maxAmount to null", async () => {
    const result = await beforeChangeHook({
      data: {
        commissionRules: [
          {
  ...
  it("should reject negative top-level maxAmount", () => {
    expect(() =>
      beforeChangeHook({
        data: {
          maxAmount: -1,
