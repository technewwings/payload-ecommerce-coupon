  ...
  ({ pluginConfig }: Args): PayloadHandler =>
    async (req) => {
      ...
      const isPartner =
        ...
        (Array.isArray(typedUser.role) && typedUser.role?.includes('partner')) || (Array.isArray(typedUser.roles) && typedUser.roles?.includes('partner'))

      const isAdmin =
        typedUser.role === 'admin' ||
        (Array.isArray(typedUser.role) && typedUser.role?.includes('admin')) || (Array.isArray(typedUser.roles) && typedUser.roles?.includes('admin'))
